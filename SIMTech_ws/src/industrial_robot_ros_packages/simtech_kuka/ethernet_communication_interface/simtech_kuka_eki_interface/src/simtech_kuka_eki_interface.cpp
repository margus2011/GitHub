                                              ...
                                              int &cmd_buff_len, int &routine)
{
  ...
  // if (!pos || !vel || !eff || !robot_command)
  if (!pos || !vel || !robot_command || !routine_command)
    return false;
...
void KukaEkiHardwareInterface::init()
{
  ...
  {
    ROS_INFO_STREAM_NAMED("simtech_kuka_eki_interface", "Configuring Kuka EKI hardware interface on: "
  ...
  {
    ROS_INFO_STREAM_NAMED("simtech_kuka_eki_interface", "Configuring Kuka EKI hardware interface socket timeout to "
                          << eki_read_state_timeout_ << " seconds");
  }
  else
  {
    ROS_INFO_STREAM_NAMED("simtech_kuka_eki_interface", "Failed to get EKI socket timeout from parameter server (looking "
  ...
  // {
  //   ROS_INFO_STREAM_NAMED("simtech_kuka_eki_interface", "Configuring Kuka EKI hardware interface maximum command buffer "
  //                         "length to " << eki_max_cmd_buff_len_);
  // }
  // else
  // {
  //   ROS_INFO_STREAM_NAMED("simtech_kuka_eki_interface", "Failed to get EKI hardware interface maximum command buffer length "
...
{
  ROS_INFO_NAMED("simtech_kuka_eki_interface", "Starting Kuka EKI hardware interface...");

  // Start client
  ROS_INFO_NAMED("simtech_kuka_eki_interface", "... connecting to robot's EKI server...");
  ...
  if (!eki_read_state(joint_position_, joint_velocity_, eki_cmd_buff_len_, routine_command_))
  {
  ...
  ROS_INFO_NAMED("simtech_kuka_eki_interface", "... done. EKI hardware interface started!");
}
...
void KukaEkiHardwareInterface::read(const ros::Time &time, const ros::Duration &period)
{
  ...
  // ROS_INFO_STREAM_NAMED("simtech_kuka_eki_interface", "Received message from hardware interface");
}
